...
const CryptoDetails = () => {
  ...
  const {data, isFetching: isCryptoDetailFetching} = useGetCryptoDetailsQuery(coinId)
  const {data: coinHistory, isFetching: isHistoryFetching} = useGetCryptoHistoryQuery({coinId, timePeriod})
  const cryptoDetails = data?.data?.coin

  useEffect(()=>{
    console.log(isHistoryFetching)
  ...
  return (
      ...
      isCryptoDetailFetching && isHistoryFetching ? (
        <Loader />
      ) : (
        <Col className="coin-detail-container">
          ...
          {!isCryptoDetailFetching && !isHistoryFetching ? (
            <LineChart 
              ...
              currentPrice={cryptoDetails?.price || 0} 
